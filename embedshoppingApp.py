...
import os
import base64
print(os.path.dirname(__file__)+"\\images")
imgpath=os.path.join(os.path.dirname(__file__),"images")
list_data=os.listdir(imgpath)
#imgfiles={}
#for fname in list_data:
#    with open(os.path.join(imgpath,fname),"rb") as fp:
#        hexa_file=fp.read()
#        imgfiles[fname]=base64.b64encode(hexa_file).decode("utf-8")
#print(len(imgfiles["y1.png"]))
#
#for fname in list_data:
#    with open(os.path.join(imgpath,"base64_"+fname),"w") as fp:
#        fp.write(imgfiles[fname])

class Js_api():
    def call_img(self,img_name):
        print(img_name)
        #return img_dataraw[img_name]
        return "나는 파이썬"
...
img_dataraw={}
for fname in list_data:
    with open(os.path.join(imgpath,fname),"r") as fp:
        if("base64_"in fname):
            img_dataraw[fname]=fp.read()
print(img_dataraw.keys())
...
<body>
    ...
    background-image:url('data:image/png;base64,%maintitle.png%')">
    ...
    <div id="mside" style="background-color:black">
        <img src="data:image/png;base64,%배너광고.png%" style="width:100%;height:100%"/>
    </div>
...
//mcenter.innerHTML=inHtml
    ...
    for(let i =0;i<evt_moniters.length;i++){
        ...
        evt_moniters[i].addEventListener("click",function(e){
            window.pywebview.api.call_img("양말이미지").then(function(pydata){
                console.log(pydata)
            })
...
"""
webview.create_window("양말 쇼핑몰",html=inHtml,js_api=api)
